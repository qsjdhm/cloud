			...
			var	$thisEl = $(this),
				...
				init = function() {
					...
					{						
						...
						$uiEl = $View(tpl.wrapper).attr({
							'id': uiId,
							'classify': $thisEl.attr('classify'),
							'requir': $thisEl.attr('requir'),
							'eType': 'select'
						});
						
						if($thisEl.attr("mapmulti")!=undefined){
							$uiEl.attr("mapmulti","true");  // 映射多选框   联动行为
						}
						
						if($thisEl.attr("mapField")!=undefined){
							$uiEl.attr("mapField","true");  // 映射联动元素的字段   联动行为
						}
						...
						// 给select类型的配置元素设置类型、ID
						//$thisEl
	
						/* Render select*/
						// 如果下拉框是字体大小下拉框   就给span设置字体
						if($thisEl.find("option:selected").attr("cSize")!=undefined && $thisEl.find("option:selected").attr("cSize")!=null){
							$sLtEl.css({ "font-size" : $thisEl.find("option:selected").attr("cSize") });
						}
						// 如果下拉框是字体样式下拉框   就给span设置字体
						if($thisEl.find("option:selected").attr("cFamily")!=undefined && $thisEl.find("option:selected").attr("cFamily")!=null){
							$sLtEl.css({ "font-size" : "16px" });
							$sLtEl.css({ "font-family" : $thisEl.find("option:selected").attr("cFamily") });
						}
						...
						$optsEl.each(function(i){
							var	optEl = $(this),
								...
								optionVal = optEl.attr("value"),
								/* Render option */
								$optionEl = $View(tpl.option).append($View(tpl.optionInner).text(optionTxt));
								$optionEl.attr("value",optionVal);
							
								/* Option event */
								// 如果下拉框是字体大小下拉框   就给span设置字体
								if(optEl.attr("cSize")!=undefined && optEl.attr("cSize")!=null){
									$optionEl.find(">span").css({ "font-size" : optEl.attr("cSize") });
									$optionEl.find(">span").attr({ "cSize" : optEl.attr("cSize") });
								}
								// 如果下拉框是字体样式下拉框   就给span设置字体
								if(optEl.attr("cFamily")!=undefined && optEl.attr("cFamily")!=null){
									$optionEl.find(">span").css({ "font-family" : optEl.attr("cFamily") });
									$optionEl.find(">span").attr({ "cFamily" : optEl.attr("cFamily") });
									$optionEl.find(">span").css({ "font-size" : "15px" });
								}
						...
						$thisEl.setOptions = function(){
                            ...
                            sltPosition.top = sltPosition.top + $selectEl.outerHeight() + ($selectEl.parents(".elem-param"))[0].scrollTop;
				...
				events = $.extend({
					...
					optionClick : function() {
						if($(this).find(">span").attr("cSize")!=undefined && $(this).find(">span").attr("cSize")!=null){
							$(this).parents(".optionsBox").prev("div").find(".selectLt").css( {"font-size" : $(this).find(">span").attr("cSize")} );
						}
						if($(this).find(">span").attr("cFamily")!=undefined && $(this).find(">span").attr("cFamily")!=null){
							$(this).parents(".optionsBox").prev("div").find(".selectLt").css( {"font-family" : $(this).find(">span").attr("cFamily")} );
						}
